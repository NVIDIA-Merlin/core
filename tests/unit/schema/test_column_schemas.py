...
@pytest.mark.parametrize("d_types", [mn.float32, mn.float64, mn.uint32, mn.uint64])
def test_dtype_column_schema(d_types):
    column = ColumnSchema("name", tags=[], properties={}, dtype=d_types)
    assert column.dtype == d_types


@pytest.mark.parametrize("external_dtype, merlin_dtype", [(pd.StringDtype, mn.string)])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            ColumnSchema("col", dtype=mn.int32, properties={"domain": {"min": 0, "max": 8}}),
            ColumnSchema("col", dtype=mn.int32, properties={"domain": {"min": 0, "max": 8}}),
        ],
        [
            ColumnSchema(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            ColumnSchema("example", tags=["tag-1"], dtype=mn.float32),
            ["tag-2", Tags.CONTINUOUS],
            ColumnSchema("example", tags=["tag-1", "tag-2", Tags.CONTINUOUS], dtype=mn.float32),
        ],
    ],
)
