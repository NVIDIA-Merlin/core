...
class LocalExecutor:
    ...
    def transform_multi(
        ...
        results = []
        for graph, data_ in zip(graphs, data):
            results.append(self.transform(data_, [graph.output_node]))
    ...
    def transform(
        ...
        for node in nodes:
            ...
            output = self._combine_node_outputs(node, transformed_data, output)

        if additional_columns:
            output = concat_columns([output, data[_get_unique(additional_columns)]])

        return output

    def _build_input_data(self, node, data, capture_dtypes=False):
        """
        ...
        if node.parents_with_dependencies:
            ...
            for parent in node.parents_with_dependencies:
                ...
                parent_data = self.transform(data, [parent], capture_dtypes=capture_dtypes)
            ...
            if addl_input_cols:
                input_data = concat_columns([input_data, data[list(addl_input_cols)]])
        else:
            # If there are no parents, this is an input node,
            # so pull columns directly from root data
            input_data = data[node_input_cols + list(addl_input_cols)]
