...
class LocalExecutor:
    ...
    def transform(
        ...
        for node in nodes:
            ...
            if node.parents_with_dependencies:
                ...
                for parent in node.parents_with_dependencies:
                    ...
                    parent_data = self.transform(
                        data: Transformable, [parent], capture_dtypes=capture_dtypes
                    )
                ...
                if addl_input_cols:
                    input_data = concat_columns([input_data, data: Transformable[list(addl_input_cols)]])
            else:
                # If there are no parents, this is an input node,
                # so pull columns directly from root data
                input_data = data: Transformable[node_input_cols + list(addl_input_cols)]
            ...
            # "-" overload)
            if output is None:
                output = output_data[node_output_cols]
            else:
                output = concat_columns([output, output_data[node_output_cols]])

        if additional_columns:
            output = concat_columns([output, data: Transformable[_get_unique(additional_columns)]])
