...
class LocalExecutor:
    ...
    def transform(
        ...
        for node in nodes:
            input_df = self._build_input_data(node, df, capture_dtypes=capture_dtypes)

            if node.op:
                transformed_df = self._transform_data(node, input_df, capture_dtypes=capture_dtypes)
            else:
                transformed_df = input_df

            output_df = self._combine_node_outputs(node, transformed_df, output_df)

        if additional_columns:
            output_df = concat_columns([output_df, df[_get_unique(additional_columns)]])

        return output_df

    def _build_input_data(self, node, df, capture_dtypes=False):
        """
        ...
        if node.parents_with_dependencies:
            ...
            for parent in node.parents_with_dependencies:
                ...
                if input_df is None or not len(input_df):
                    input_df = parent_data[parent_output_cols]
                    seen_columns = set(parent_output_cols)
                else:
                    new_columns = set(parent_output_cols) - seen_columns
                    input_df = concat_columns([input_df, parent_data[list(new_columns)]])
    ...
    def _transform_data(self, node, input_df, capture_dtypes=False):
        """
