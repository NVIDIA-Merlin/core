...
class LocalExecutor:
    ...
    def transform(
        ...
        for node in nodes:
            input_df = self._build_input_df(node, df, capture_dtypes=capture_dtypes)

            if node.op:
                transformed_df = self._transform_df(node, df, capture_dtypes=capture_dtypes)
            else:
                transformed_df = input_df

            output = self._combine_node_outputs(node, transformed_df, output)

        if additional_columns:
            output = concat_columns([output, df[_get_unique(additional_columns)]])

        return output

    def _build_input_df(self, node, df, capture_dtypes=False):
        """
        ...
        -------
        DataFrameType
            The input dataframe formed from the outputs of upstream parent/dependency nodes
        """
    ...
    def _transform_df(self, node, input_df, capture_dtypes=False):
        """
        ...
        -------
        DataFrameType
            The output dataframe formed by executing the final node's transform
        ...
        RuntimeError
            If no dataframe is returned from the operator
        """
        try:
    ...
    def _combine_node_outputs(self, node, transformed_df, output):
        ...
        # "-" overload)
        if output is None:
            output = transformed_df[node_output_cols]
        else:
            output = concat_columns([output, transformed_df[node_output_cols]])
